...
pub struct Entry {
    ...
    /// Per-entry binary attachment references.
    ///
    /// Each map key is the user-facing attachment name (e.g. `"id_rsa"`); the
    /// value is the identifier (`Ref` attribute) that points into the binary
    /// pool. For KDBX4 the pool lives in `Database::header_attachments` and
    /// the identifier is the textual form of the index. For KDBX3 it would
    /// live in `Meta::binaries` and the identifier matches `BinaryAttachment::identifier`.
    ///
    /// Binaries are kept out-of-band from `fields` so that the on-wire XML
    /// matches what KeePassXC writes:
    /// ```xml
    /// <Entry>
    ///   ...
