            ...
            match event {
                SimpleXmlEvent::Start(name, _) => match &name[..] {
                    ...
                    "Binary" => {
                        ...
                        // Record the per-entry binary reference so it can round-trip on save.
                        // The identifier indexes into `Database::header_attachments` (KDBX4
                        // inner-header binary pool) or `Meta::binaries` (KDBX3 meta pool).
                        out.binaries.insert(field.key, field.identifier);
                    }
                    "AutoType" => {
