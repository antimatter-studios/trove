...
| Real `<Binary>` attachments (KeePassXC interop) | ✅ | KeePassXC opens our vaults; non-UTF-8 bytes round-trip; verified in [crates/trove-core/tests/binary_attachments.rs](../crates/trove-core/tests/binary_attachments.rs) |

### Entry management

| Feature | State | Notes |
|---|---|---|
| Add entry to root group | ✅ | [Vault::add_entry](../crates/trove-core/src/lib.rs) |
...
| `ssh-add -d` (client-driven removal) | ❌ | Not exposed; trove controls the key list |
...
### GPG agent — `troved` Assuan socket

| Feature | State | Notes |
|---|---|---|
| `git commit -S` end-to-end | ✅ | Real `gpg` against our socket; verified in [crates/troved/tests/gpg_git_signing_e2e.rs](../crates/troved/tests/gpg_git_signing_e2e.rs) |
...
| Feature | State | Notes |
|---|---|---|
| `Materialize.{Source,Target,Mode,TTL,AllowDiskBacked}` schema | ✅ | [crates/troved/src/materialize/](../crates/troved/src/materialize/) |
| Write file with explicit mode on unlock | ✅ | |
| Wipe on lock or TTL expiry (random overwrite + fsync + truncate + unlink) | ✅ | [crates/troved/src/materialize/wipe.rs](../crates/troved/src/materialize/wipe.rs) |
...
| Feature | State | Notes |
|---|---|---|
| Default 15 min, env override (`TROVE_IDLE_TIMEOUT`), RPC override | ✅ | |
...
5. **Daemon lifecycle.** `troved &` starts it; nothing keeps it running across reboots yet. No launchd / systemd unit.
...
1. **Keyfile unlock** (v0.0.11.0). Plumb `DatabaseKey::with_keyfile(reader)` through `Vault::open` and the `unlock` RPC. CLI: `trove unlock <vault> --keyfile <path>`. ~30 minutes of work plus tests.
2. **YubiKey challenge-response unlock** (v0.0.11.1). Plumb `DatabaseKey::with_challenge_response_key`. Larger lift — requires the `challenge_response` feature on `keepass`, which pulls in `nusb`. CLI: `trove unlock <vault> --yubikey-slot <N>`.
3. **launchd plist + systemd unit file** (v0.0.12.0). Ship `packaging/launchd/com.semdatex.troved.plist` and `packaging/systemd/troved.service`. Daemon survives reboot / login.
4. **`trove doctor`** (v0.0.12.1). Pre-flight: daemon running, socket reachable, vault format detected, keyfile path resolvable, expected env vars set. Saves users from "why doesn't this work" trial-and-error.
5. **`trove import-ssh ~/.ssh`** (v0.0.13.0). Convenience: scan `~/.ssh/`, prompt per key, copy any user-confirmed keys into the vault as new entries with the right attachment naming. Lets users bootstrap a fresh trove vault from their existing keys without manual `trove add ssh` per key.
6. **Better `trove status`** (v0.0.13.1). Today reports counts; should also list per-entry: which keys are loaded into SSH agent, which materializations are live, with their target paths. `--verbose` surfaces non-loadable attachments and why.
...
20. **HIBP continuous monitoring + breach alerts.** Background-checked, surfaced via `trove health` and (later) GUI notifications.
...
26. **Better Linux keyring integration** (Secret Service, kwallet, gnome-keyring) so other apps can drive trove transparently.
...
- **Upstream contributions to `keepass-rs`.** The clean-room test suite in [crates/keepass-spec-tests](../crates/keepass-spec-tests/) is intended as a PR. The `trove-patches-v0.7.33` branch on https://github.com/antimatter-studios/keepass-rs holds the pre-migration state for reference.
- **Fuzz coverage for the kdbx parser.** Same shape as [crates/troved/fuzz/](../crates/troved/fuzz/). Lives in keepass-spec-tests when added.
...
|---|---|---|
| `trove-core` integration (vault round-trip, attachments) | 11 | |
| `troved` lib unit (idle, ssh wire, assuan, pkdecrypt, materialize) | 66 | |
| `troved` daemon e2e (ping/shutdown, ssh-add, ssh-sign, gpg-sign, gpg-decrypt, materialize, idle, status) | ~25 | spread across 9 test binaries |
| `trove-cli` integration | 2 | unlock+status against an in-process daemon |
