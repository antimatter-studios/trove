...
//!   * The socket is bound at sdpmd startup, before any vault is unlocked.
...
mod tests {
    ...
    fn resolve_ssh_socket_honours_explicit_override() {
        ...
        let prev = std::env::var("SDPM_SSH_SOCK").ok();
        std::env::set_var("SDPM_SSH_SOCK", "/tmp/explicit-sdpm-ssh.sock");
        let p = resolve_ssh_socket_path();
        assert_eq!(p, PathBuf::from("/tmp/explicit-sdpm-ssh.sock"));
        match prev {
            Some(v) => std::env::set_var("SDPM_SSH_SOCK", v),
            None => std::env::remove_var("SDPM_SSH_SOCK"),
        }
    }
}
