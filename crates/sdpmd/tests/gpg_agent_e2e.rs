...
async fn pksign_full_round_trip_signs_and_verifies() {
    ...
    // SETKEYDESC ... → OK
    wh.write_all(b"SETKEYDESC sdpm-test\n").await.unwrap();
    let lines = read_until_terminator(&mut reader).await;
    assert_eq!(lines, vec!["OK"]);

    // SETHASH 8 <hex of "hello-sdpm" sha-256-stand-in> → OK
    ...
    let raw = sdpmd::gpg_agent::assuan::percent_decode(encoded).expect("percent decode");
