...
//! that two semver-incompatible `keepass` crate versions (0.12.5 and 0.13.13)
...
fn keepass_013_links_and_round_trips() {
    ...
    db.save(&mut buf, DatabaseKey::new().with_password(PW))
        .expect("0.13.13 save");
    assert!(!buf.is_empty(), "0.13.13 produced empty bytes");
    Database::open(
        &mut std::io::Cursor::new(&buf),
        DatabaseKey::new().with_password(PW),
    )
    .expect("0.13.13 reopen");
}

/// Cross-version read: a vault written by 0.12.5 must open under 0.13.13 (this is
...
fn vault_written_by_012_opens_under_013() {
    ...
    .expect("0.13.13 must read a 0.12.5-written vault");
}
